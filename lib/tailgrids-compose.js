...
function isCompositionFaithful(jsx, cvaSpec) {
  ...
  if (/<[A-Z][\w]*[\s/>]/.test(jsx)) return false;
  return true;
}

// Compose a full preview given the raw registry + preview sources and
// a theme map. Returns { jsx, html } when the result is faithful;
...
export function composePreview({ registrySource, previewSource, themeMap }) {
  ...
  if (!isCompositionFaithful(finalJsx, cvaSpec)) return null;
  return {
    jsx: finalJsx,
