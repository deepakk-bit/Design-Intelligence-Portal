...
const REFERO_URL =
  process.env.REFERO_MCP_URL ?? "https://api.refero.design/mcp";
...
async function rpc(method, params, { useSession = true } = {}) {
  ...
  if (!res.ok) {
    const detail =
      ...
      (payload ? JSON.stringify(payload).slice(0, 400) : "(empty body)");
    const msg = `refero ${method} failed: HTTP ${res.status} @ ${REFERO_URL} — ${detail}`;
    console.error(`[refero] ${msg}`);
  ...
  if (payload?.error) {
    const msg = `refero ${method} rpc error: ${payload.error.message ?? JSON.stringify(payload.error).slice(0, 200)}`;
    console.error(`[refero] ${msg}`);
    const err = new Error(msg);
