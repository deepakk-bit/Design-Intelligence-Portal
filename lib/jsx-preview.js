...
// Build the .jsx payload for the card-header Copy button. Returns the
// default state's component file exactly as the model produced it —
// no leading comment block, no trailing commented-out states. The
// downstream Figma plugin's parser scans the head of the input for a
// JSX tag and bails on anything else (it reported "No JSX found" when
// we prefixed the file with `//` metadata), so we keep the payload
// uncluttered. Other states stay reachable via the per-section copy
// chips inside the output card.
export function assembleCombinedFile({ sections }) {
  const first = sections?.[0];
  if (!first?.jsx) return "";
  return String(first.jsx).trim();
}
