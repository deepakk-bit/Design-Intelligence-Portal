...
export async function handleAnalyze(req, res) {
  ...
  //   2. agent.defaultModel
  //   3. global MODEL env var
  // The select-type validation already constrained `cleanedExtras.modelTier`
  // to one of the agent's declared options, so it's safe to use directly.
  const callModel =
    cleanedExtras.modelTier || agent.defaultModel || MODEL;
  if (userContext) {
...
export async function handleChat(req, res) {
  ...
  let chatModel = agent.defaultModel || MODEL;
  if (Array.isArray(agent.extras) && extras && typeof extras === "object") {
    for (const def of agent.extras) {
      ...
      if (def.type === "select") {
        const allowed = def.options.map((o) => o.value);
        if (typeof raw === "string" && allowed.includes(raw)) chatModel = raw;
      }
    }
  }
