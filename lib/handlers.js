...
// Map a Refero screen object onto the frontend shape. We're defensive about
// field names because Refero's response shape isn't fully spec'd in their
// public docs — accept several aliases for each concept.
function normalizeReference(r) {
  if (!r || typeof r !== "object") return {};
  const id = r.screen_id ?? r.id ?? r.uuid ?? null;
  const imageUrl =
    r.preview_url ?? r.image_url ?? r.imageUrl ?? r.image ??
    r.screenshot_url ?? r.thumbnail_url ?? r.thumbnail ?? null;
  const sourceUrl =
    r.refero_url ?? r.permalink ?? r.source_url ?? r.url ??
    (id ? `https://refero.design/screens/${id}` : null);
  const productRaw =
  ...
  const product =
    typeof productRaw === "object"
      ...
      : productRaw ?? "";
  const category = r.category ?? r.screen_type ?? r.type ?? "";
  const title =
