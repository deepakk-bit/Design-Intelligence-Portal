...
import { captureScreenshot, fetchPageDigest } from "./screenshot.js";
...
// Render the HTML digest as compact markdown for the model.
function formatPageDigest(d) {
  const lines = ["# Page digest"];
  if (d.title) lines.push(`- **Title**: ${d.title}`);
  if (d.description) lines.push(`- **Meta description**: ${d.description}`);
  lines.push(
    `- **Counts**: images=${d.counts.images} (missing-alt=${d.counts.imagesMissingAlt}, empty-alt=${d.counts.imagesEmptyAlt}), buttons=${d.counts.buttons}, links=${d.counts.links}, aria-labels=${d.counts.ariaLabels}, roles=${d.counts.roles}`,
  );
  if (d.headings?.length) {
    lines.push("\n## Heading hierarchy");
    for (const h of d.headings.slice(0, 30)) {
      lines.push(`- ${h.level}: ${h.text}`);
    }
  }
  const noAlt = d.images?.filter((i) => !i.hasAlt).slice(0, 12) ?? [];
  if (noAlt.length) {
    lines.push("\n## Images missing alt attribute (sample)");
    for (const i of noAlt) lines.push(`- ${i.src || "(no src)"}`);
  }
  return lines.join("\n");
}
...
export async function handleAnalyze(req, res) {
  ...
  // QA Report pre-processing: capture a live screenshot of the URL and a
  // small HTML digest, then add them to the model's context. The cleanComponent
  // string holds the URL (textInputKind="url"); userContext holds the scope.
  if (agent.kind === "qa-report") {
    let parsedUrl;
    try {
      parsedUrl = new URL(cleanComponent);
      if (!/^https?:$/.test(parsedUrl.protocol)) throw new Error("not http(s)");
    } catch {
      return sendJson(res, 400, {
    ...
    try {
      const [shot, digest] = await Promise.all([
        captureScreenshot(parsedUrl.href),
        fetchPageDigest(parsedUrl.href).catch((err) => ({
          _error: err.message ?? "page fetch failed",
        })),
      ]);
      orderedImages.push({
        data: shot.data,
        mediaType: shot.mediaType,
        label: "Live (rendered, desktop)",
      });
      // Re-label the design ref image as "Design reference" so the model
      // sees them in clear order.
      if (orderedImages[0]) orderedImages[0].label = "Design reference";
      const digestText = digest._error
        ? `# Page digest\n(unavailable: ${digest._error})`
        : formatPageDigest(digest);
      const scopeText = userContext
        ? `# Scope\n${userContext}`
        : "# Scope\n(none — full-page review)";
      // Override userText: the agent.userInstruction is included via parts
      // earlier, but for this agent we want a richer prompt.
      payload.__qaReportExtra = `${scopeText}\n\n# URL\n${parsedUrl.href}\n\n${digestText}`;
    } catch (err) {
      console.error("[qa-report] capture failed:", err);
      return sendJson(res, 502, {
        error: `Couldn't capture the live page: ${err.message ?? "unknown"}`,
      });
    }
  }

  let userText;
  if (agent.kind === "qa-report" && payload.__qaReportExtra) {
    // Custom assembly for QA Report — the URL, scope, and page digest
    // replace the default "# Component / # Additional context" wrappers.
    userText = `${agent.userInstruction}\n\n${payload.__qaReportExtra}`;
  } else {
