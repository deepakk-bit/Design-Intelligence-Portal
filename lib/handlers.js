...
import { buildSketchFileForComponent } from "./tailgrids-sketch.js";
import {
  postProcessJsxGenResult,
  unwrapJsxGenForChat,
} from "./jsx-preview.js";
import {
  HAS_DB,
...
// Generate a .sketch file for a TailGrids component. The component
// is fetched the same way the canvas preview is, then handed to the
// Sketch emitter — Phase 2 promotes repeated primitives into Symbol
// Masters with Smart Layout, so resizing in Sketch reflows the
// instance content the same way Figma's Auto Layout would.
//
// Query: ?slug=<component-id> (e.g. button, badge, alert-dialog)
export async function handleTailgridsSketch(req, res) {
  if (!requireAuth(req, res)) return;
  const url = new URL(req.url, "http://x");
  const slug = (url.searchParams.get("slug") || "").trim();
  if (!slug) {
    return sendJson(res, 400, { error: "slug query param required" });
  }
  try {
    const comp = await fetchTailgridsComponent(slug);
    if (!comp) {
      return sendJson(res, 404, { error: `unknown component: ${slug}` });
    }
    if (!Array.isArray(comp.sections) || comp.sections.length === 0) {
      return sendJson(res, 422, {
        error:
          "no composable sections for this component — Sketch export is only available for components that render in the live preview.",
      });
    }
    const buf = await buildSketchFileForComponent(comp);
    const filename = `${slug}.sketch`;
    res.writeHead(200, {
    ...
    console.error("[tailgrids-sketch] error:", err);
    sendJson(res, 500, { error: err.message ?? "sketch build failed" });
  }
}
...
export async function handleAnalyze(req, res) {
  ...
  // The API key check happens *after* we know the agent, so deterministic
  // agents that don't call Anthropic (TailGrids generator) can run on
  // servers without an API key configured.
  if (!client && agent.kind !== "tailgrids") {
  ...
  // Generator. Phase 1: fetch the picked component's raw .tsx source
  // straight from the upstream repo and return it. No Anthropic call,
  ...
  if (agent.kind === "tailgrids") {
    ...
    let comp;
    try {
      comp = await fetchTailgridsComponent(picked);
    } catch (err) {
      return sendJson(res, 502, {
        error: `Failed to fetch TailGrids component '${picked}': ${err.message ?? "network error"}`,
      });
    }
    if (!comp) {
      return sendJson(res, 404, {
        error: `TailGrids component not found: ${picked}`,
      });
    }
    return sendJson(res, 200, {
      ...
      result: {
        tailgrids: {
          ...
          previewSource: comp.previewSource,
          previewSourceUrl: comp.previewSourceUrl,
          // Phase 2: html/jsx are populated when composition succeeded
          // (single-cva components today). Null when the component is
          // multi-cva or non-cva — the UI then falls back to a
          // source-only view.
          html: comp.html,
          jsx: comp.jsx,
          // For source-only components: every upstream preview .tsx
          // joined with banner comments. UI uses this as the Copy
          // payload when jsx is null so users still get usage code
          // (Alert variants, Toast variants, etc.) instead of the
          // raw component definition.
          previewSourceJoined: comp.previewSourceJoined,
        },
      },
  ...
  try {
    ...
    // React+Tailwind Component Generator: take the schema-shaped JSON
    // (componentName, description, sections[]) and render per-section
    // preview HTML plus the combined copy payload. The canvas detects
    // the result by the presence of `r.jsxGen` so we wrap it here.
    if (agent.id === "jsx-generator") {
      parsed = postProcessJsxGenResult(parsed);
    }
...
export async function handleChat(req, res) {
  ...
  // jsx-generator stores its result wrapped under `jsxGen` (with extra
  // server-derived fields like `html` per section and `combinedJsx`).
  // The model only knows the flat schema shape, so unwrap before
  // replaying the prior turn back to it.
  const initialForModel =
    agentId === "jsx-generator" && initialResult?.jsxGen
      ? unwrapJsxGenForChat(initialResult)
      : initialResult;
  ...
  if (seedImages.length > 0) {
    ...
    if (initialForModel) {
      conversation.push({
        role: "assistant",
        content: [
          { type: "text", text: JSON.stringify(initialForModel, null, 2) },
        ],
      });
    }
    ...
    if (initialForModel) {
      conversation.push({
        role: "assistant",
        content: JSON.stringify(initialForModel, null, 2),
      });
    }
  }
  ...
  try {
    ...
    // jsx-generator round-trip: rebuild per-section HTML and the
    // combined copy payload so the canvas can re-render the refined
    // output the same way it rendered the initial result.
    if (agentId === "jsx-generator" && updatedResult) {
      updatedResult = postProcessJsxGenResult(updatedResult);
    }
...
// --- Library (per-device-uuid scoped save collection) ----------------------

// Library responses go cross-origin: the Figma plugin's UI iframe (origin
// `null` for srcdoc) and any other allowed deployment of the web app
// both call these endpoints from a different origin than the API host.
// We allow `*` because the routes are already scoped by the
// X-Library-Code header — no cookies, no credentials, nothing to leak
// to a third-party origin beyond what the library code itself reveals.
function setLibraryCors(res) {
  res.setHeader("Access-Control-Allow-Origin", "*");
  res.setHeader("Access-Control-Allow-Methods", "GET, POST, DELETE, OPTIONS");
  res.setHeader(
    "Access-Control-Allow-Headers",
    "X-Library-Code, Content-Type",
  );
  res.setHeader("Access-Control-Max-Age", "86400");
  // Library data is per-user-code and changes on every save. Tell any
  // intermediary (Railway edge, browser cache, CDN, etc.) not to cache,
  // and to vary by the library code header in case caching is forced
  // upstream. Without this a fresh GET right after a POST can return
  // the previous empty payload from cache, making the just-saved
  // component look like it didn't persist.
  res.setHeader("Cache-Control", "no-store, no-cache, must-revalidate");
  res.setHeader("Vary", "X-Library-Code");
}

export function handleLibraryOptions(_req, res) {
  setLibraryCors(res);
  res.writeHead(204);
  res.end();
}

// Read + validate the X-Library-Code header. The library code is the user's
// identity; missing or malformed = 401. We don't enforce a strict UUID shape
// (the web app generates with crypto.randomUUID, but old browsers may write
// shorter codes), just a sane length/charset.
function readLibraryCode(req) {
  const raw = req.headers?.["x-library-code"];
...
function requireDb(res) {
  if (!HAS_DB) {
    sendJson(res, 503, {
      error:
        "POSTGRES_URL not configured on server. Provision a Postgres instance and set POSTGRES_URL.",
    });
    return false;
  }
  return true;
...
function requireLibraryCode(req, res) {
  const code = readLibraryCode(req);
  if (!code) {
    sendJson(res, 401, {
      error:
        "Missing or invalid X-Library-Code header. Open the web app once to generate a code, or paste an existing one in the pairing panel.",
    });
    return null;
  }
  return code;
...
function reportLibraryError(res, err) {
  if (err?.code === "MISSING_LIBRARY_CODE") {
    return sendJson(res, 401, { error: err.message });
  }
  if (err?.code === "EMPTY_SECTIONS") {
...
export async function handleLibraryList(req, res) {
  setLibraryCors(res);
  if (!requireAuth(req, res)) return;
  if (!requireDb(res)) return;
  const code = requireLibraryCode(req, res);
  if (!code) return;
  try {
    const saves = await listSaves(code);
    return sendJson(res, 200, { saves });
  } catch (err) {
    return reportLibraryError(res, err);
  }
}

export async function handleLibraryCreate(req, res) {
  setLibraryCors(res);
  if (!requireAuth(req, res)) return;
  if (!requireDb(res)) return;
  const code = requireLibraryCode(req, res);
  ...
  try {
    ...
    if (err?.code === "PAYLOAD_TOO_LARGE") {
      return sendJson(res, 413, { error: "save payload too large" });
    }
    return sendJson(res, 400, { error: "invalid request body" });
  }
  try {
    const save = await createSave(code, payload);
    return sendJson(res, 201, { save });
  } catch (err) {
    return reportLibraryError(res, err);
  }
}

export async function handleLibraryGet(req, res, { id }) {
  setLibraryCors(res);
  if (!requireAuth(req, res)) return;
  if (!requireDb(res)) return;
  const code = requireLibraryCode(req, res);
  if (!code) return;
  try {
    const save = await getSave(code, id);
    if (!save) return sendJson(res, 404, { error: "not found" });
    return sendJson(res, 200, { save });
  } catch (err) {
    return reportLibraryError(res, err);
  }
}

export async function handleLibraryDelete(req, res, { id }) {
  setLibraryCors(res);
  if (!requireAuth(req, res)) return;
  if (!requireDb(res)) return;
  const code = requireLibraryCode(req, res);
  if (!code) return;
  try {
    const ok = await deleteSave(code, id);
