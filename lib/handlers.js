...
export async function handleAnalyze(req, res) {
  ...
  try {
    ...
    // with the canonical shadcn preset when the requested component
    // matches one we know. This guarantees real shadcn tokens end up in
    // the rendered matrix and the copied JSX, instead of approximations.
    // Falls back to whatever the model produced when there's no preset.
    if (agent.id === "states-variants" && cleanComponent) {
      const preset = getShadcnPreset(cleanComponent);
      if (preset) {
