...
import { searchScreens, getScreen, listTools, HAS_REFERO } from "./refero.js";
...
async function safeGetScreen(id, fallback) {
  try {
    const detail = await getScreen(id);
    return { ...fallback, ...detail };
  } catch (err) {
    console.warn(`[refero] get_screen(${id}) failed:`, err.message);
    return fallback;
  }
}

// Map a screen object (from search_screens or get_screen) to the shape the
// frontend renders. We accept several common field aliases.
function normalizeReference(r) {
  if (!r || typeof r !== "object") return {};
  const id = r.id ?? r.screen_id ?? r.uuid ?? null;
  const imageUrl =
    r.image_url ?? r.imageUrl ?? r.image ?? r.thumbnail_url ?? r.thumbnail ??
    r.preview_url ?? null;
  const sourceUrl =
    r.source_url ?? r.sourceUrl ?? r.url ?? r.permalink ??
    r.refero_url ?? (id ? `https://refero.design/screens/${id}` : null);
  const product =
    r.product ?? r.app ?? r.brand ?? r.company ?? r.source ?? null;
  const title =
    r.title ?? r.name ?? r.label ??
    [product, r.screen_type, r.category].filter(Boolean).join(" — ") ?? "";
  return {
    id,
    title: title || "Untitled",
    product: typeof product === "object" ? product?.name ?? "" : product ?? "",
    category: r.category ?? r.screen_type ?? "",
  ...
  try {
    ...
    if (agent.kind === "references") {
      ...
      try {
        ...
        const searchPayload = await searchScreens(query, 12);
        const candidates = collectScreenSummaries(searchPayload).slice(0, 8);
        // Hydrate top hits in parallel for full image URLs / metadata.
        const detailed = await Promise.all(
          candidates.map(async (c) =>
            c.id ? safeGetScreen(c.id, c) : c,
          ),
        );
        const references = detailed
        ...
        return sendJson(res, 200, {
          ...
          result: {
            query,
            queryType: parsed.queryType ?? "screens",
            references,
          },
          usage: {
