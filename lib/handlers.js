...
export async function handleAnalyze(req, res) {
  ...
  const { agentId, image, context, componentName } = payload;
  if (!agentId) {
  ...
  const inputs = agent.inputs ?? ["image"];
  ...
  if (wantsText && !wantsImage && !cleanComponent) {
    return sendJson(res, 400, {
...
export async function handleChat(req, res) {
  ...
  const { agentId, image, initialResult, messages, componentName, context } =
    payload;
