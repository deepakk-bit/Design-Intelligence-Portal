...
export async function handleAnalyze(req, res) {
  ...
  // Per-agent extras (e.g. "library" dropdown for States & Variants).
  // We validate against the agent's declared `extras` shape and stuff the
  // sanitized values into the user prompt as a labeled block. Unknown keys
  // are dropped silently — the schema field is the source of truth.
  const cleanedExtras = {};
