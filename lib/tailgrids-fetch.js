...
import {
  renderAlertSections,
  buildAlertStackedHtml,
} from "./tailgrids-alert-render.js";
...
export async function fetchTailgridsComponent(slug) {
  ...
  let jsx = null;
  let previewSections = [];
  // Raw preview sources, kept regardless of compose success. Source-
  // only components (Alert, Toast, ...) use this for their Copy
  // payload so users get usage examples instead of the registry
  // definition.
  let allPreviewFiles = [];
  ...
  try {
    ...
    for (const spec of specs) {
      ...
      // Always keep the raw preview source. For components that
      // compose, the showcase JSX is built from this; for components
      // that don't, Copy uses these files concatenated so users still
      // get usable React code (e.g. <Alert variant="success" ... />)
      // rather than the underlying component definition.
      allPreviewFiles.push({
        label: spec.label,
        filename: `${spec.filename}.tsx`,
        source: previewResult.text,
        sourceUrl: previewResult.url,
      });
    ...
    if (previewSections.length > 0) {
      ...
      jsx = buildSelfContainedReactFile({
        slug,
    ...
    } else if (slug === "alert" && allPreviewFiles.length > 0) {
      // Alert is multi-cva and the static composer skips it, but we
      // have a hand-rolled renderer that resolves each `<Alert>` call
      // in the preview source against the theme map and produces a
      // stacked HTML doc the iframe can paint. JSX stays empty —
      // Copy still uses previewSourceJoined for that path.
      const alertSections = renderAlertSections(allPreviewFiles, themeMap);
      if (alertSections.length > 0) {
        html = buildAlertStackedHtml(alertSections);
      }
    }
    // Sort preview files so the index matches the MDX-defined order
    // (specs already iterate in that order, so allPreviewFiles is
    // already correct).
  } catch (err) {
  ...
  // For components that don't compose to a renderable preview
  // (Alert, Toast, etc.), build the Copy payload from the raw
  // upstream preview files instead. Devs get usage examples like
  // `<Alert variant="success" title="..." />` rather than the
  // Alert component definition itself — closer to what they want
  // to paste into a TailGrids-set-up app.
  const previewSourceJoined = allPreviewFiles.length
    ? buildPreviewFilesJoined(allPreviewFiles)
    : null;
  ...
  const value = {
    ...
    sections: previewSections,
    // Every preview file fetched from upstream, regardless of
    // whether it composed. Available to the UI as a Copy fallback.
    previewFiles: allPreviewFiles,
    previewSourceJoined,
  };
...
// Build a fully self-contained React component file. Inlines every
// TailGrids primitive used by the showcase (with theme tokens
// pre-resolved to hex) plus the showcase itself, so the clipboard
// contents paste-compile in a blank React + Tailwind + class-variance-
// authority project. No `@/components/tailgrids/core/...` imports, no
// theme CSS variables, no missing dependencies.
//
// Output shape:
//
//   "use client";                      // when any source had one
...
//   }
//
//   const buttonStyles = cva("...", { variants: {...}, compoundVariants: [...] });
//   export function Button(...) { ... }
//
//   // ...any sibling primitives the showcase uses, same treatment...
//
...
//     );
//   }
function buildSelfContainedReactFile({
  slug,
  sections,
  primarySource,
  siblingSources,
  ...
  // Collect every registry source used by the showcase. Always start
  // with the primary registry, then add siblings referenced from the
  // preview files.
  const usedSlugs = new Set([slug]);
  for (const sec of sections) {
    for (const imp of detectAllComponentImports(sec.rawSource ?? "")) {
      usedSlugs.add(imp.slug);
    }
  }
  const allSources = { ...siblingSources, [slug]: primarySource };

  // Inline each registry source: resolve theme tokens in every string
  // literal, drop the `cn` import (we provide an inline helper),
  // strip any external icon/asset imports we can't honour, and
  // collect surviving npm-package imports for the file header.
  const externalImports = new Map(); // key → line
  const inlined = [];
  let useClientNeeded = false;

  for (const usedSlug of usedSlugs) {
    const src = allSources[usedSlug];
    if (!src) continue;
    if (/["']use client["']/.test(src)) useClientNeeded = true;
    const { body, imports } = inlineRegistrySource(src, themeMap);
    for (const imp of imports) {
      externalImports.set(imp.replace(/\s+/g, " ").trim(), imp);
    }
    inlined.push(body.trim());
  }

  // Collect imports from the preview files too — icon packages,
  // notably. Drop registry imports since we've inlined those, drop
  // assets we can't reach, drop `import type` re-exports.
  for (const sec of sections) {
    ...
    for (const line of lines) {
      ...
      if (/from\s*["']@\/registry\/core\//.test(stripped)) continue;
      if (/from\s*["']@\/utils\/cn["']/.test(stripped)) continue;
      if (/from\s*["']@\/assets\//.test(stripped)) continue;
      if (/from\s*["']react-aria-components["']/.test(stripped)) continue;
      if (/from\s*["']@base-ui\//.test(stripped)) continue;
      if (/^import\s+type\b/.test(stripped)) continue;
      externalImports.set(stripped.replace(/\s+/g, " "), stripped);
    }
  }

  // Body: each section's JSX, dedented and reindented under the
  // default export. Theme tokens inside every string literal (most
  // commonly `className="..."`) get resolved to arbitrary-value
  // form so the showcase paints with no theme CSS configured.
  function resolveStrings(s) {
    return s.replace(
      /("[^"\\]*(?:\\.[^"\\]*)*")|('[^'\\]*(?:\\.[^'\\]*)*')/g,
      (match, dq, sq) => {
        const quote = dq != null ? '"' : "'";
        const inner = dq != null ? dq.slice(1, -1) : sq.slice(1, -1);
        return quote + resolveClassNames(inner, themeMap) + quote;
      },
    );
  }
  ...
  for (const sec of sections) {
    ...
    if (!body) continue;
    const resolved = resolveStrings(body);
    bodyChunks.push(
      `${" ".repeat(6)}{/* ${sec.label} */}\n${indentLines(dedentLines(resolved), 6)}`,
    );
  }
  ...
  const innerBody =
    bodyChunks.length === 1
      ? indentLines(
          dedentLines(resolveStrings(extractPreviewJsxBody(sections[0].rawSource))),
          4,
        )
      : `    <>\n${bodyChunks.join("\n\n")}\n    </>`;

  const useClient = useClientNeeded ? '"use client";\n\n' : "";
  const importBlock = [...externalImports.values()].join("\n");
  const cnHelper = [
    "// Tiny class-merger — same surface as `clsx` / `cn` so the cva",
    "// outputs below compose cleanly.",
    "function cn(",
  ...
  return [
    useClient + importBlock + (importBlock ? "\n\n" : ""),
    cnHelper,
    "",
    inlined.join("\n\n"),
    "",
...
// Inline one registry .tsx: resolve theme-token classes to hex
// arbitrary values in every string literal, drop the `cn` import
// (the caller provides one), and capture surviving external imports
// for hoisting to the top of the combined file.
//
// Returns `{ body, imports }` where `body` is the rewritten source
// (everything except the imports we relocated) and `imports` is the
// list of import statements to hoist.
function inlineRegistrySource(source, themeMap) {
  // Split off the import block (top-of-file lines that start with
  // `import`). Stop at the first non-import, non-blank, non-comment
  // line.
  ...
  while (i < lines.length) {
    ...
    if (/^\s*import\b/.test(line)) {
      // Multi-line import block: read until the closing `;` or end-of-line
      // with a `from "..."` pattern. Most TailGrids imports are single
      // line but a few span multiple lines.
      let acc = line;
      while (
        !/["'][^"']+["'];?\s*$/.test(acc) &&
        i + 1 < lines.length
  ...
  // Filter & rewrite imports.
  const keptImports = [];
  for (const line of importLines) {
    if (/from\s*["']@\/utils\/cn["']/.test(line)) continue;
    if (/from\s*["']@\/registry\/core\//.test(line)) continue; // inlined below
    ...
    keptImports.push(line.trim());
  }

  // Resolve theme-token classes inside every "..."/'...' string literal
  // in the body. cva variant strings live in literals — this catches
  // them all without trying to parse JS structure.
  const rewritten = remaining.replace(
    /("[^"\\]*(?:\\.[^"\\]*)*")|('[^'\\]*(?:\\.[^'\\]*)*')/g,
    (match, dq, sq) => {
      const quote = dq != null ? '"' : "'";
      const inner = dq != null ? dq.slice(1, -1) : sq.slice(1, -1);
      const resolved = resolveClassNames(inner, themeMap);
      return quote + resolved + quote;
    },
  );

  return { body: rewritten, imports: keptImports };
}
...
// For source-only components: concatenate every upstream preview .tsx
// into one paste-friendly block. Each section gets a banner comment
// with the demo's label + the upstream filename so devs know which
// preview each block came from.
//
// We also rewrite `@/registry/core/<slug>` import paths to
// `@/components/tailgrids/core/<slug>` — the registry path is
// monorepo-internal; the components path is what the CLI writes
// into a user's project.
function buildPreviewFilesJoined(files) {
  const blocks = files.map((f) => {
    const banner =
      `// ─────────────────────────────────────────────\n` +
      `// ${f.label} · ${f.filename}\n` +
      `// ─────────────────────────────────────────────\n\n`;
    const rewritten = f.source.replace(
      /@\/registry\/core\//g,
      "@/components/tailgrids/core/",
    );
    return banner + rewritten.trim();
  });
  return blocks.join("\n\n");
}
