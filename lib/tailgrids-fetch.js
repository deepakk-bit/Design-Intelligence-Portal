...
export async function fetchTailgridsComponent(slug) {
  ...
  try {
    ...
    if (previewSections.length > 0) {
      ...
      jsx = buildStackedJsx(previewSections);
    }
  } catch (err) {
...
// Build the JSX payload for the Copy CTA. Stacks every section with
// an HTML comment header so devs can see which TailGrids demo each
// block came from — same ordering tailgrids.com uses.
function buildStackedJsx(sections) {
  if (sections.length === 1) return sections[0].jsx;
  return sections
    .map((s) => `{/* ${s.label} */}\n${s.jsx}`)
    .join("\n\n");
}
