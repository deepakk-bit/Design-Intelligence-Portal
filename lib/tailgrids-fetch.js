...
import { composePreview } from "./tailgrids-compose.js";
...
export async function fetchTailgridsComponent(slug) {
  ...
  let jsx = null;
  try {
    const previewUrl = `${PREVIEW_BASE}/${slug}/${slug}-preview.tsx`;
    const previewResult = await fetchFirstAvailable([previewUrl]);
    if (previewResult) {
